  ...
  <div style="height: 405px">
    <div style="height: 321px">
      <el-scrollbar max-height="321px" ref="scrollbarRef">
        <p v-for="item in rt.recruit_information" :key="item">
          {{ item }}
        </p>
      </el-scrollbar>
    </div>
    <div class="recruitment-bottom">
      <div class="recruitment-bottom-left">
        <el-row :gutter="40" style="margin: 0">
          <el-col :span="12">
            <el-checkbox label="自动设置时间" v-model="config.tools!.recruit.params.set_time" />
          </el-col>
          <el-col :span="8">
...
const star3Change460 = (value: boolean) => {
  ...
  if (value) {
    ...
    config.tools!.recruit.params.recruitment_time['3'] = 460
  } else if (!value && star3_time60.value) {
    config.tools!.recruit.params.recruitment_time['3'] = 60
  } else {
    config.tools!.recruit.params.recruitment_time['3'] = 540
  }
}
...
const changeSelect = () => {
  ...
  if (select3.value && !config.tools!.recruit.params.select.includes(3)) {
    config.tools!.recruit.params.select.push(3)
  }
  if (!select3.value && config.tools!.recruit.params.select.includes(3)) {
    config.tools!.recruit.params.select = config.tools!.recruit.params.select.filter(
      (item: number) => item !== 3
    )
  }

  if (select4.value && !config.tools!.recruit.params.select.includes(4)) {
    config.tools!.recruit.params.select.push(4)
  }
  if (!select4.value && config.tools!.recruit.params.select.includes(4)) {
    config.tools!.recruit.params.select = config.tools!.recruit.params.select.filter(
      (item: number) => item !== 4
  ...
  if (select5.value && !config.tools!.recruit.params.select.includes(5)) {
    config.tools!.recruit.params.select.push(5)
  }
  if (!select5.value && config.tools!.recruit.params.select.includes(5)) {
    config.tools!.recruit.params.select = config.tools!.recruit.params.select.filter(
      (item: number) => item !== 5
  ...
  if (select6.value && !config.tools!.recruit.params.select.includes(6)) {
    config.tools!.recruit.params.select.push(6)
  }
  if (!select6.value && config.tools!.recruit.params.select.includes(6)) {
    config.tools!.recruit.params.select = config.tools!.recruit.params.select.filter(
      (item: number) => item !== 6
...
const start = async () => {
  config.tools!.recruit.params.enable = true
  if (rt.recruit_listen) {
    rt.recruit_listen()
  }
  config.status = 1
  rt.recruit_listen = await listen(config.name + '_tools_recruit_handle', (event: any) => {
    const payload = event.payload as Payload
    if (payload.msg.includes('RecruitResult: ')) {
      rt.recruit_information.push(payload.msg.split('RecruitResult: ')[1])
      changeText()
    }
    if (payload.code !== 0) {
      config.status = 0
  ...
  await ToolsExecute(config.name, 'recruit', { tasks: [config.tools!.recruit] })
}

onMounted(() => {
  if (config.tools!.recruit.params.select.includes(3)) {
    select3.value = true
  }
  if (config.tools!.recruit.params.select.includes(4)) {
    select4.value = true
  }
  if (config.tools!.recruit.params.select.includes(5)) {
    select5.value = true
  }
  if (config.tools!.recruit.params.select.includes(6)) {
    select6.value = true
  }
